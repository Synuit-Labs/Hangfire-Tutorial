...
namespace RouteDelivery.OptimizationEngine
{
    public class OptimizationEngine: IOptimizationEngine
    {
        ...
        public List<DeliverySchedule> OptimizeDeliveries(int requestID)
        {
            ...
            foreach (var c in customers)
            {
                ...
                foreach (var d in deliveries.Where(d => d.CustomerID == c.ID))
                {
                    ...
                    if (idealDriver != null)
                    {
                        optimizedScheduled.Add(new DeliverySchedule() { CustomerID = c.ID,
                            DriverName = idealDriver.DriverName, OptimizationRequestID = request.ID,
                            PackageID = d.ID, TransportType = d.TransportType,
                            EstimatedTime = request.ScheduleDate.AddHours(deliveryNo), ID = deliveryNo});
                    }
                }
            }
        ...
        private Driver GetIdealDriver(IEnumerable<Driver> drivers, Models.Type.TransportType transportType, 
