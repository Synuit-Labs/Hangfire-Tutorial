        ...
        public ActionResult Create(OptimizationRequest newOptimizationRequest)
        {
            if (ModelState.IsValid)
            {
                ...
                RecurringJob.AddOrUpdate(newOptimizationRequest.ID.ToString(), 
                    () => _optiEngine.OptimizeDeliveries(new OptimizeDeliveriesRequest() {  RequestID = newOptimizationRequest.ID, ScheduleDate = newOptimizationRequest.RequestDate}), 
                    cronType);
        ...
        public ActionResult Edit(OptimizationRequest OptimizationRequestEdit)
        {
            if (ModelState.IsValid)
            {
                ...
                RecurringJob.AddOrUpdate(OptimizationRequestEdit.ID.ToString(),
                    () => _optiEngine.OptimizeDeliveries(new OptimizeDeliveriesRequest() { RequestID = OptimizationRequestEdit.ID, ScheduleDate = OptimizationRequestEdit.RequestDate }),
                    cronType);
                return RedirectToAction("Index");
            }
